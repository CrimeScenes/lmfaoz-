...
RunService.RenderStepped:Connect(function()
    if TriggerBotEnabled and IsTargeting and TargetPlayer then
        if IsAlignedWithCamera(TargetPlayer) then
            ...
            -- Example: Fire a weapon only at the targeted player
            print("TriggerBot activated on: " .. TargetPlayer.Name)
            -- Add your shooting logic here (this is where you'd use Silent to shoot the TargetPlayer)
        end
    end
end)
...
RunService.RenderStepped:Connect(function()
    if IsTargeting and TargetPlayer and TargetPlayer.Character then
        ...
        if BodyPart then
            ...
            if SilentEnabled then -- Ensure Silent is enabled
                -- Fire your silent shooting action here, using the target's position
                print("Silent shooting at: " .. TargetPlayer.Name)
                -- Example: Fire a weapon at BodyPart position
            end
        end
    end
end)
...
Run_Service.RenderStepped:Connect(function()
    if getgenv().Silent.Startup.Enabled and IsTargeting and TargetPlayer then  -- Ensure CamLock is engaged and there's a valid target
        UpdateFOV()  -- Call UpdateFOV to refresh visibility
        
        -- Use the currently targeted player from CamLock
...
local function HookTool(tool)
    if tool:IsA("Tool") then
        tool.Activated:Connect(function()
            if TargetPlayer and TargetPlayer.Character and tick() - lastToolUse > 0.1 then  -- Debounce for 0.1 seconds
                lastToolUse = tick()
                local closestPart, closestPoint = GetClosestHitPoint(TargetPlayer.Character)  -- Use TargetPlayer from CamLock
                if closestPart and closestPoint then
                    local velocity = GetVelocity(TargetPlayer, closestPart.Name)  -- Ensure it uses the correct target
                    Replicated_Storage[RemoteEvent]:FireServer(Argument, closestPoint + velocity * getgenv().Silent.Startup.Prediction)
                end
            end
        end)
